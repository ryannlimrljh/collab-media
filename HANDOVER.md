...
| `audience-marks.css` / `audience-marks.js` | The drawings: `AudienceMarks.arc(el, segment)` renders the consumption rings, `AudienceMarks.jar` is the jar physics: `build(segments, lens, stage)`, `step(J, dt, hand)`, `settle(J, steps)`, `resize(J, stage)`, `setActive(J, ids)`, `setLens(J, lens)`. Pure functions, no DOM in the layout, so the planner's audience step can reuse them |

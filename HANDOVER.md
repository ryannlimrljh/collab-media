...
| `formats.html` | Ad formats (General) | One page, two halves. On top, the page drawn as a picture: eight ad slots marked where they really sit on a desktop page and a phone, each carrying how many units fit it and each demonstrating itself on hover. Clicking one filters the catalogue below and scrolls to it. Under it, the catalogue: the 49 KULT units with a live sample on every card, objective/family/size/device/placement filters, gallery and table views, a spec drawer, and a compare tray of up to four. Deep links: `?slot=<key>`, `?f=<id>` (`?tab=catalogue` still lands on the catalogue). Design note: `docs/superpowers/specs/2026-09-07-formats-catalogue-design.md` |
| `formats-original.html` | — (not linked) | Archive: Ad formats as it stood at commit `5639207`, before the placement picture and the sample rework of 2026-09-09. Kept so the two can be put side by side. Loads frozen copies of the three shared files that changed since, from `shared/archive/2026-09-08/`; the shell and the DLS are the live ones, since those did not change. Open it directly — nothing links to it — and do not edit it |
| `audiences.html` | Audiences (General) | The audience catalogue in two tabs. Universe: 51 KULT segments as flat discs poured into a fixed jar, colour deepening with audience size, stirred by the pointer, three lenses over eight groups. Segments: cards with a drawn illustration per segment that animates on hover, revealing the total audience and its media mix as bars, a table, a drawer, and a reach tray of up to five with Karen's overlap rule and refine filters. Design note: `docs/superpowers/specs/2026-09-08-audiences-catalogue-design.md` |
...
| `formats-data.js` | The format catalogue: specs transcribed from kult.my/gallery (2026-09-07) merged with the rate card's `bestFor` / `cpm` / `inventory`. The nine standard display units are catalogue defaults, flagged `source: "catalogue"`. Also `FORMAT_PLACEMENTS` (the eight slots) and, derived onto every record at load, `placements` (every slot it can fill) and `home` (the slot it was built for) — a curation from the sizes, not a KULT-published field |
...
| `audience-marks.css` / `audience-marks.js` | The drawings:  `AudienceMarks.jar` is the jar physics: `build(segments, lens, stage)`, `step(J, dt, hand)`, `settle(J, steps)`, `resize(J, stage)`, `setActive(J, ids)`, `setLens(J, lens)`. Pure functions, no DOM in the layout, so the planner's audience step can reuse them |

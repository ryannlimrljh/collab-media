...
| `formats.html` | Ad formats (General) | Two tabs. **Placements** opens on the page drawn as a picture: eight slots marked where they really sit on a desktop page and a phone, each carrying how many units fit it, each demonstrating itself on hover, each opening the catalogue filtered to it. **Catalogue** is the 49 KULT units with a live sample on every card, objective/family/size/device/placement filters, gallery and table views, a spec drawer, and a compare tray of up to four. Deep links: `?tab=catalogue`, `?slot=<key>`, `?f=<id>`. Design note: `docs/superpowers/specs/2026-09-07-formats-catalogue-design.md` |

...
| `audience-marks.css` / `audience-marks.js` | The drawings: `AudienceMarks.arc(el, segment)` renders the consumption rings, `AudienceMarks.layoutUniverse(segments, lens, stage, style)` lays the circles out as rings, a network or one pack. Pure functions, no DOM in the layout, so the planner's audience step can reuse them |

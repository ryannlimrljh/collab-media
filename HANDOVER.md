...
| `audience-marks.css` / `audience-marks.js` | The drawings: `AudienceMarks.arc(el, segment)` renders the consumption rings, `AudienceMarks.layoutSpace(segments, lens, stage, style)` fills the canvas with the bubbles by group or by size and gives each a depth. Pure functions, no DOM in the layout, so the planner's audience step can reuse them |
